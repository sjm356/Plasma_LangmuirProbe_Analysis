...
v2.3: Adding else route for the condition that the ion current is positive and floating potential cannot be found.
      ...
      Changing determination of plasma potential from first largest dI/dV number to n th larget dI/dV number for excepting error values at starting voltage
v2.4: Changing the method of getting floating potential. determinating Vf from smoothed data.
      Voltage range for geting ion current fitting is set by constant voltage values
      Adding determining the voltage range for linear fitting curve for getting Te
      Adding probe setup output file
v2.5: Adding code for change NaN data to number data in raw IV data set 
      
v3.0: Method for getting electron temperature is moving curve fitting and finding minimum R-square of linear fitting
...
new_file_name_Probe = "Probe setup"
new_file_Total = open(new_file_path + "/" + new_file_name_Total + ".dat",'w')
new_file_Probe = open(new_file_path + "/" + new_file_name_Probe + ".txt",'w')
new_file_Total.write(f'Filename \t Vf(IV) \t Vp(IV) \t Vp(SG) \t ne(IV) \t Te \n')
new_file_Probe.write(f'Probe tip length (m): {L} \n Probe tip Radius (m): {R} \n Probe tip area (m2): {Ap} \n Probe shape: {Shape}')
new_file_Probe.close()
...
#######!!!!!!! CONFIRM current + -!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        raw_I = IV[:,1]
        raw_I = np.nan_to_num(raw_I)
        mov_avg_IV = signal.filtfilt(w, 1, raw_I)   # smoothing raw IV data for lowering noise
        for k, V in enumerate(mov_avg_IV):
            if V == 0:  # if there is zero current, floating potential is determined at that point
                Vf = float(V)
                #print(Vf)
                break
            elif (mov_avg_IV[k-1] * mov_avg_IV[k] < 0): # if there is cross point between IV curve and voltage axis, floating potential is determined at that cross point
                if (mov_avg_IV[k-1] < mov_avg_IV[k]):
                    Vf = float((IV[k-1][0] + IV[k][0])/2)
                    #print(Vf)
                    break
            else :  # if there is not both zero current and cross point(All the current is positive), floating potential is set by negative value (meaninigless value)
                Vf = - 1
        
        ## Fitting and Subratcting Ion current
            # Defining fitting range
#################################################################################
################## !!!!!!!!!Starting and Ending voltage is set by constant values
#################################################################################
        '''
        ...
        '''
        id_V_ion_1 = np.argmin(np.abs(IV[:,0] - (- 50)))    # starting voltage is -50
        id_V_ion_2 = np.argmin(np.abs(IV[:,0] - (- 0)))    # Ending voltage is 0
            # Extracting IV range
        ion_V = IV[int(id_V_ion_1):int(id_V_ion_2),0]
        ion_I = IV[int(id_V_ion_1):int(id_V_ion_2),1]
        ion_V = np.nan_to_num(ion_V)
        ion_I = np.nan_to_num(ion_I)
            # Linear fitting tool
        def func(x, a, b):
            return a*x + b
...
#################################################################################
###########!!!!!!!!! Determination of plasma potential from dI/dV!!!!!!!!!!!!!!!!
#################################################################################
        srt_mov_avg = np.argsort(-mov_avg)  #sorting array with decending order
        for id_Vp_temp in srt_mov_avg:
            if IV[id_Vp_temp,0] > 0:    # Only getting positive plasma potential (excepting error at starting voltage region) 
                id_Vp = id_Vp_temp
                Vp = float(IV[id_Vp_temp,0])    # Plasma potential
                Ie = float(IV[id_Vp_temp,2])    # Electron saturation current
                break
        '''    
        ...
        '''
        Noise_amp_abs = float((max(I_noni[int(id_V_ion_1):int(id_V_ion_2)]-min(I_noni[int(id_V_ion_1):int(id_V_ion_2)]))) / 2)
...
#################################################################################
        # Determination of the range of voltage for getting curv fitting values
        # Te is determined by floating potential and the difference with plasma potential
        ln_I = np.log(I_noni[:,0])
        ln_I = np.nan_to_num(ln_I)
        
        Te_V = IV[int(id_V_ion_2 + 30):id_Vp,0]
        Te_lnI = ln_I[int(id_V_ion_2  + 30):id_Vp]
        
        r_squared = list()
        popt_list = []
        for ddd, dVV in enumerate(Te_lnI):
            n_dVV = 27
            if ddd + n_dVV >= len(Te_lnI):
                break
            if dVV < -100:
                Te_lnI[ddd] = -100
            Te_dlnI = Te_lnI[ddd : ddd + n_dVV]
            Te_dV = Te_V[ddd : ddd + n_dVV]
            popt_fit_Te = curve_fit(func, Te_dV, Te_dlnI)
            popt_list.append(popt_fit_Te[0])
            residuals = Te_dlnI - func(Te_dV, np.nan_to_num(popt_fit_Te[0][0]), np.nan_to_num(popt_fit_Te[0][1]))
            ss_res = np.sum(residuals ** 2)
            ss_tot = np.sum((Te_dlnI - np.mean(Te_dlnI)) ** 2)
            r_squared.append(1 - (ss_res / ss_tot))
        id_max_r2 = np.argmax(np.nan_to_num(r_squared))
        Te = 1 / popt_list[id_max_r2][0]
        ne = Ie / e / Ap / np.sqrt(Te * e / 2 / np.pi / m_e) / 1000000
        
        '''
        if Vf > 0:  # if Vf is positive, voltage range is determined by the difference between Vp and Vf
            up_lim = Vf + (Vp - Vf) / 3 * 2
            low_lim = Vf + (Vp - Vf) / 2
        else :  # if Vf is negative or well not determined, voltage range is determined just by the Vp only
            up_lim = Vp - 0.5
            low_lim = up_lim - 3
        ...
        k1_min = SG_param_new[id_min_SG_param[0][0], 0]
        f1_min = SG_param_new[id_min_SG_param[0][0], 2]
        ...
        #opt_sg0_new_IV = signal.savgol_filter()
        ...
        Vp_new = IV[id_Vp_new,0]    # Plasma Potential from smoothed IV
