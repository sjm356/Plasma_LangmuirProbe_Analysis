...
new_file_name_Total = "Plasma"
new_file_Total = open(new_file_path + "/" + new_file_name_Total + ".dat",'w')
new_file_Total.write(f'Filename \t Vp(IV) \t Vp(SG) \t ne(IV) \t Te \n')
...
if len(file_list) == 0:
    ...
    for iiii in file_list:
        ...
        V_ion_1 = -1 * min(abs(IV[:,0] - (Vf - 40))) + (Vf - 40)    #adjusting range
        ...
        # Saving & Writing data
        new_file_name = "V1_EEDF_" + str(iiii)
        new_file_name_IV = "V1_IV_" + str(iiii)
        ...
        new_file_Total.write(f'{iiii} \t {Vp} \t {Vp_new} \t {ne} \t {Te} \n')
        new_file.close()
        new_file_IV.close()
